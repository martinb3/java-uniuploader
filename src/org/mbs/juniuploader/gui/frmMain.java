...
import org.mbs.juniuploader.StatusThread;
import org.mbs.juniuploader.objects.UAInterface;
import org.mbs.juniuploader.objects.localobjects.WDirectory;
import org.mbs.juniuploader.objects.remoteobjects.FormPairGroup;
import org.mbs.juniuploader.objects.remoteobjects.UploadRule;
import org.mbs.juniuploader.objects.remoteobjects.UploadSite;
import org.mbs.juniuploader.util.*;
...
public class frmMain extends javax.swing.JFrame {
    ...
    public static void doAllUploads(final JButton btn)
    {
        ...
        org.mbs.juniuploader.StatusThread.addMessage("Spinning off upload threads for every rule");
        ...
        Runnable go = new Runnable() {
            public void run() {
                ...
                for(int i = 0; i < uploadRules.getSize(); i++)
                {
                    ...
                    if(ur.isValid())
                    {
                        org.mbs.juniuploader.StatusThread.addMessage("Uploading selected file to " + ur.getUploadSite().getUrl());
                        Util.postFileUpload(ur);
                        org.mbs.juniuploader.StatusThread.addMessage("Completed an upload to " + ur.getUploadSite().getUrl());
                    }
                }
                
                org.mbs.juniuploader.StatusThread.addMessage("All uploads threads completed!");
                if(!btn.isEnabled())
                    btn.setEnabled(true);
            }
        };
