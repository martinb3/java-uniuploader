...
public class Prefs
{
    ...
    public static void exportFile(File fOut)
    {
        ...
        {
            org.mbs.juniuploader.StatusThread.addMessage("Attempting to export settings to " + fOut.getName());
            ...
            try {
                ...
                org.mbs.juniuploader.StatusThread.addMessage("Export of settings to " + fOut.getName() + " completed");
            } catch (Exception ex)
            {
                log.warn("exportFile() thought everything was fine, but an exception occurred", ex);
                org.mbs.juniuploader.StatusThread.addMessage("Export settings to " + fOut.getName() + " caused an exception");
            }
        }
        else
        {
            log.debug("exportFile() failed, the specified file was null/unwriteable");
            org.mbs.juniuploader.StatusThread.addMessage("Export settings to " + fOut.getName() + " failed, file was null or unwritable");
        }
    }
